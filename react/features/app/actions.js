...
export function maybeRedirectToWelcomePage(options: Object = {}) {
    return (dispatch: Dispatch<any>, getState: Function) => {
       ...
       //      new PostMessageTransportBackend({
        //            postisOptions: { scope: 'cc' }
         //       })
            
                const DEFAULT_POSTIS_OPTIONS = {
                    window: window.opener || window.parent
                };
         //   alert(JSON.stringify(interfaceConfig))
 
      //   DEFAULT_POSTIS_OPTIONS.window.postMessage(JSON.stringify({po:true,scope:'cc'}), "*");
         
   //         window.opener.postMessage(JSON.stringify({po:true,scope:'cc'}),"*");
    //    window.parent.postMessage(JSON.stringify({po:true,scope:'cc'}),  "*");

    window.parent.postMessage("end",  "*");
        //    parent.location.href="https://www.tenu.me/profile"
           /* setTimeout(
                () => {
              //     alert("redirect ??? ")
